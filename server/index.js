...
app.get("/api/product", (req, res) => {
    ...
    if (!name && !category) {
        ...
        let count = found.slice(offset).slice(0, limit);
        res.json({
            products: count,
            count: found.length,
            total: Math.ceil(found.length / limit)
        });
        ...
        let found = products.products.filter((el) =>
        ...
        let found = products.products.filter(
            (el) => el.title.toLowerCase().includes(name) && el.category == category
        );
